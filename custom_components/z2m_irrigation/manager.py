...
class ValveManager:
    ...
    async def start(self):
        # Subscribe to ALL known response topics across Z2M versions
        for t in (
            f"{self.base}/bridge/config/devices",
            f"{self.base}/bridge/devices",
            f"{self.base}/bridge/response/device/list",
        ):
            await mqtt.async_subscribe(self.hass, t, self._devices_msg)

        # Fire requests for a device list (both styles)
        await mqtt.async_publish(self.hass, f"{self.base}/bridge/config/devices/get", "")
        await mqtt.async_publish(self.hass, f"{self.base}/bridge/request/device/list", json.dumps({"id":"ha"}))

        # Also honor manual topics immediately
        for b in self._manual_bases:
            await self._ensure_valve(b)

    async def _ensure_valve(self, base: str, name: Optional[str]=None):
        key = base.strip("/")
        if key in self.valves: return
    ...
    async def _devices_msg(self, msg):
        try:
            ...
            return
        devices = payload if isinstance(payload, list) else payload.get("data") or payload.get("devices") or []
        for d in devices:
    ...
    async def publish_state(self, base: str, on: bool):
        v = self.valves.get(base.strip("/"))
        if not v: return
        await mqtt.async_publish(self.hass, f"{v.base}/set", json.dumps({"state": "ON" if on else "OFF"}))
        if not on:
            v.litres_target = None
            if v.off_handle: v.off_handle.cancel(); v.off_handle = None

    async def turn_on_for(self, base: str, minutes: int):
        await self.publish_state(base, True)
        v = self.valves.get(base.strip("/")); 
        if not v: return
        if v.off_handle: v.off_handle.cancel(); v.off_handle = None
        loop = asyncio.get_running_loop()
        def _off():
            asyncio.create_task(self.publish_state(v.base, False))
            v.off_handle = None
    ...
    async def turn_off(self, base: str):
        await self.publish_state(base, False)
